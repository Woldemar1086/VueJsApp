...
// For login
	...
	it('Missed password',() => {
		...
		expect(validators(model[1]).errors.password).toBe('Write your password')
	});

	it('Missed password and repassword',() => {
		expect(validators(model[2]).isValid).toBe(false)
		expect(validators(model[2]).errors.password).toBe('Write your password')
	});

	it('Not valid email',() => {
		expect(validators(model[5]).isValid).toBe(false)
		expect(validators(model[5]).errors.email).toBe('Format of your email is not valid')
	});
...
// For registration
	...
	it('Not valid email',() => {
		...
		expect(validators(model[5], key).errors.email).toBe('Format of your email is not valid')
	});

	it('Missed repassword',() => {
		expect(validators(model[3], key).isValid).toBe(false)
		expect(validators(model[3], key).errors.equal).toBe('Passwords are not equal')
	});

	it('Passwords are not equal',() => {
		expect(validators(model[6], key).isValid).toBe(false)
		expect(validators(model[6], key).errors.equal).toBe('Passwords are not equal')
	});

	it('Password too short',() => {
		expect(validators(model[7], key).isValid).toBe(false)
		expect(validators(model[7], key).errors.passwordLength).toBe('Your password is too short. It should be longer than 8 symbols.')
	});
})